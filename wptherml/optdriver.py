...
class OptDriver(TmmDriver):
    ...
    def optimize_qubo(self, fom_function):
        ...
        if self.combinatorial_optimization == True:
            self.optimizer = self._qubo_combinatorial_structure_optimization(self, self.optimization_dictionary, fom_function)
            self.optimizer.learning_loop(num_to_train=25, num_iterations=10000)

        if self.qubo_thickness_optimization == True:
            
            self.optimizer = self._qubo_thickness_and_alloy_optimization(self, optimization_dict=self.optimization_dictionary, fom_func = fom_function)
            self.optimizer.learning_loop(num_to_train=100, n_epochs = 250, l2_lambda=0.001, num_iterations=400, reduction_factor=2,  l1_lambda =0.000001, K = 25, LR = 0.01)
    ...
    def optimize_bfgs(self):
        ...
        fom_start, grad_start = self.compute_fom_and_gradient_from_thickness_array(x_start)
        print(F' Initial FOM is {fom_start}')
        print(F' Initial Gradient is {grad_start}')

        ret = minimize(self.compute_fom_and_gradient_from_thickness_array, x_start, method="L-BFGS-B", jac=True, bounds=bfgs_bounds)
        print(ret.x)
    ...
    class _qubo_thickness_and_alloy_optimization:
        ...
        def __init__(self, outer_instance, optimization_dict: dict, fom_func):
            ...
            self.thickness_adjustment_dict = optimization_dict['thickness_adjustment_dict']
            ...
            for key in self.bit_split_dict:
                ...
                if 'T' in self.bit_split_dict[key]:
                    self.ranges['T'].append({key: [0, self.thickness_adjustment_dict[key]* self.thickness_list[key]]})
        ...
        def learning_loop(self, num_iterations=1, num_to_train=400, plot_train_data=True, reduction_factor=2, n_epochs = 60, l2_lambda = 0.0001, l1_lambda = 0, K = 5, LR= 0.05):
            ...
            for i in range(0, num_iterations):
                ...
                fm.train( np.array(df.loc[:, 'bitstrings']).tolist(), np.array(df.loc[:, 'FOM']).reshape(-1,1), split_size = 0.8, batch_size = 10000, n_epochs = n_epochs, minmaxscale=True, LR = LR, l2_lambda=l2_lambda, l1_lambda= l1_lambda, opt = "LBFGS")
                    ...
                    for j in range(0, len(self.thickness_list)):
                        ...
                        for l in range(0, len(self.ranges['A'])):
                            ...
                            if(j in self.ranges['A'][l]):
                                center_of_new_range = qubo_result_df.loc[0,str("alloy_indice_" + str(j))]

                                new_range = ((self.ranges['A'][l][j][1] - self.ranges['A'][l][j][0]) / reduction_factor)

                                print(min(1,2))
                                print(max(1,2))
                                self.ranges['A'][l][j][0] = min(max(0, (center_of_new_range - (new_range/2))),1)
                                self.ranges['A'][l][j][1] = min(max(0,(center_of_new_range + (new_range/2))),1)
                        ...
                        for l in range(0, len(self.ranges['T'])):
                            ...
                            if(j in self.ranges['T'][l]):
                                ...
                                center_of_new_range = qubo_result_df.loc[0,str("thickness_indice_" + str(j))]
                                new_range = ((self.ranges['T'][l][j][1] - self.ranges['T'][l][j][0]) / reduction_factor)

                                self.ranges['T'][l][j][0] = min(max(0, (center_of_new_range - (new_range/2))),1)
                                self.ranges['T'][l][j][1] = min(max(0,(center_of_new_range + (new_range/2))),1)
            ...
            for i in range(0, len(bitstrings)):
                ...
                for indice in self.bit_split_dict:
                    ...
                    for key2 in self.bit_split_dict[indice]:
                        ...
                        bitstring_seg = bitstrings[i][current_bit:current_bit+self.bit_split_dict[indice][key2]]
                        ...
                        if(key2 == 'A'):
                            ...
                            _n1 = np.copy(self.og_test._refractive_index_array[:,self.alloy_dict[indice][0]])
                            _n2 = np.copy(self.og_test._refractive_index_array[:,self.alloy_dict[indice][1]])


                            n_eff = self.MaxwellGarnett(_n1, _n2, fw)
                            self.outer_instance._refractive_index_array[:,self.alloy_dict[indice][0]] = n_eff
                        ...
                        if(key2 == 'T'):
                            ...
                            self.test_args['Thickness_List'][indice] = range_y[0] + (self.og_test_args['Thickness_List'][indice] * percent) * (range_y[1] - range_y[0])/(self.og_test_args['Thickness_List'][indice] * scaling_factor )
                            self.outer_instance.thickness_array[indice] = range_y[0] + (self.og_test_args['Thickness_List'][indice] * percent) * (range_y[1] - range_y[0])/(self.og_test_args['Thickness_List'][indice] * scaling_factor )
                            

                            new_row_for_df.append(self.test_args['Thickness_List'][indice])
    ...
    class _qubo_combinatorial_structure_optimization:
            ...
            for key in optimization_dict:
                ...
                for material in materials:
                    if material == "SiO2":
                        ...
                        materials_methods_inner.append(self.outer_instance.material_HfO2)
                    elif material == "AlN":
                        materials_methods_inner.append(self.outer_instance.material_AlN)
                    elif material == "Al2O3":
                        materials_methods_inner.append(self.outer_instance.material_Al2O3)
                    elif material == "TiO2":
                        materials_methods_inner.append(self.outer_instance.material_TiO2)
                    elif material == "TiN":
                        materials_methods_inner.append(self.outer_instance.material_TiN)
                    elif material == "ZrO2":
                        materials_methods_inner.append(self.outer_instance.material_ZrO2)
                    elif material == "Si3N4":
                        materials_methods_inner.append(self.outer_instance.material_Si3N4)
        ...
        def learning_loop(self, num_iterations=10, num_to_train=100, n_epochs = 60, l2_lambda = 0.0001, l1_lambda = 0, K = 5, LR= 0.05):
