...
class Therml:
    ...
    def _compute_therml_spectrum(self, wavelength_array, emissivity_array):
        ...
        self.blackbody_spectrum /= ( np.exp( h * c / (wavelength_array * kb * self.temperature ) ) - 1 )
    ...
    def _compute_power_density(self, wavelength_array):
        ...
        blackbody_spline = UnivariateSpline(wavelength_array, self.blackbody_spectrum, k=3)

        # fit cubic spline to thermal emission spectrum
        thermal_emission_spline = UnivariateSpline(wavelength_array, self.thermal_emission_array)
